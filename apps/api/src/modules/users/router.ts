...
import { FollowUserParamsSchema, FollowListQuerySchema } from "../follow/schemas.js"
...
export const usersRouter: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get("/me/education", { preHandler: [requireAuth] }, async (request, reply) => {
    if (!request.user) return
    try {
  ...
  fastify.post("/me/experience", { preHandler: [requireAuth] }, async (request, reply) => {
    if (!request.user) return
  ...
  fastify.put("/me/experience/:expId", { preHandler: [requireAuth] }, async (request, reply) => {
    if (!request.user) return
    ...
    try {
      const exp = await updateExperience(request.user.userId, params.data.expId, parsed.data)
  ...
  fastify.delete("/me/experience/:expId", { preHandler: [requireAuth] }, async (request, reply) => {
    if (!request.user) return
    const params = ExperienceIdParamsSchema.safeParse(request.params)
    if (!params.success) {
      return reply.status(400).send({
  ...
  // ── Skills ──────────────────────────────────────────────────────────────────
  fastify.post("/me/skills", { preHandler: [requireAuth] }, async (request, reply) => {
    if (!request.user) return
  ...
  fastify.delete("/me/skills/:skillId", { preHandler: [requireAuth] }, async (request, reply) => {
    if (!request.user) return
  ...
  fastify.get("/:userId/followers", { preHandler: [requireAuth] }, async (request, reply) => {
  ...
  fastify.get("/:userId/following", { preHandler: [requireAuth] }, async (request, reply) => {
    const params = FollowUserParamsSchema.safeParse(request.params)
    if (!params.success) {
      return reply.status(400).send({
        error: "Invalid user ID",
