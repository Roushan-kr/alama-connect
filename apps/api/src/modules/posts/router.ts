...
export const postsRouter: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.post("/:contentId/like", { preHandler: [requireAuth] }, async (request, reply) => {
    if (!request.user) return
  ...
  fastify.delete("/:contentId/like", { preHandler: [requireAuth] }, async (request, reply) => {
    if (!request.user) return
    const params = ContentIdParamsSchema.safeParse(request.params)
    if (!params.success) {
      return reply.status(400).send({
  ...
  fastify.post("/:contentId/comments", { preHandler: [requireAuth] }, async (request, reply) => {
    if (!request.user) return
    const params = ContentIdParamsSchema.safeParse(request.params)
    if (!params.success) {
      return reply.status(400).send({
    ...
    try {
      const result = await createComment(request.user.userId, params.data.contentId, parsed.data)
      return reply.status(201).send({ data: result })
    } catch (err: unknown) {
      return handleError(err, reply)
    }
  ...
  fastify.get("/:contentId/comments", { preHandler: [requireAuth] }, async (request, reply) => {
    const params = ContentIdParamsSchema.safeParse(request.params)
    if (!params.success) {
      return reply.status(400).send({
        error: "Invalid content ID",
...
export const commentsRouter: FastifyPluginAsync = async (fastify) => {
  fastify.delete("/:commentId", { preHandler: [requireAuth] }, async (request, reply) => {
    if (!request.user) return
