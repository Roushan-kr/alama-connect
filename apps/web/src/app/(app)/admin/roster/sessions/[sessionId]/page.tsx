...
export default function RosterSessionPage({ params }: { params: Promise<{ sessionId: string }> }) {
  ...
  const { data: sessionData, error } = useQuery<any>({
  ...
  // Sync mappings with detected headers when session becomes SANITIZED
  useEffect(() => {
    if (session?.status === "SANITIZED" && session?.mergeSummary?.detectedHeaders) {
      ...
      const initial = headers.map((header: string) => {
        ...
        let coreField: string | undefined;
        ...
        // CamelCase conversion for templateVar
  ...
  if (!session) {
    return <div className="text-sm text-slate-500 animate-pulse">Loading session details...</div>;
  }
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-1">
            File: <span className="font-semibold">{session.originalName}</span> &bull; Status:{" "}
            <span className="font-bold text-brand-600 uppercase">{session.status}</span>
          </p>
        </div>
        ...
        {session.status === "MAPPED" && (
          <button
            ...
            className="rounded-xl bg-brand-600 hover:bg-brand-700 px-5 py-2.5 text-xs font-bold text-white shadow transition-all"
          >
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
            <div className="p-4 bg-slate-50 rounded-xl border border-slate-100">
              <span className="text-[10px] font-bold text-slate-400 block uppercase">Total Uploaded</span>
              <span className="text-xl font-extrabold text-slate-900">{session.mergeSummary.totalRows || 0} rows</span>
            </div>
            <div className="p-4 bg-slate-50 rounded-xl border border-slate-100">
              <span className="text-[10px] font-bold text-slate-400 block uppercase">Clean Rows</span>
              <span className="text-xl font-extrabold text-slate-900">{session.mergeSummary.cleanRows ?? session.mergeSummary.inserted ?? 0}</span>
            </div>
            <div className="p-4 bg-slate-50 rounded-xl border border-slate-100">
              <span className="text-[10px] font-bold text-slate-400 block uppercase">Sanitization Errors</span>
              <span className="text-xl font-extrabold text-slate-900">{session.mergeSummary.errorCount || 0}</span>
            </div>
            <div className="p-4 bg-slate-50 rounded-xl border border-slate-100">
              <span className="text-[10px] font-bold text-slate-400 block uppercase">Flagged Removed</span>
              <span className="text-xl font-extrabold text-slate-900">{session.mergeSummary.flaggedRemoved || 0}</span>
            </div>
          </div>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm space-y-4">
          <div>
            ...
            <p className="text-[10px] text-slate-500 mt-0.5">
              Preview shows raw column detection. Final validation runs after you confirm mappings.
            </p>
          </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-slate-100 text-[10px] uppercase font-bold text-slate-400">
                  ...
                  <th className="pb-3">Template Variable name</th>
                  <th className="pb-3">Type</th>
                  <th className="pb-3">Structured Schema Field</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-50 text-xs text-slate-700">
                {mappings.map((mapping, idx) => (
                  <tr key={idx}>
                    <td className="py-3 font-semibold">{mapping.excelHeader}</td>
                    <td className="py-3">
                      <input
                        type="text"
                        value={mapping.templateVar}
                        onChange={(e) => handleFieldChange(idx, "templateVar", e.target.value)}
                        className="rounded-lg border border-slate-200 px-2.5 py-1.5 text-xs text-slate-800 focus:border-brand-500 outline-none w-40"
                      />
                    </td>
                    <td className="py-3">
                      <select
